// Dual Stream Audio Transcription Service
// Captures and transcribes both user microphone and meeting audio simultaneously
...
export const useDualStreamTranscription = () => {
  ...
  const [meetingVolume, setMeetingVolume] = useState(1.0);
  ...
  const captureUserAudio = async () => {
    try {
      ...
      setError('Failed to access user microphone. Please check permissions.');
      return null;
    }
  };
  ...
  const updateDisplayTranscript = () => {
    ...
    const recentText = recentSegments
      ...
      .map(segment => {
        const prefix = segment.source === 'user' ? '[You]' : '[Meeting]';
  ...
  const initializeRecognition = async () => {
    ...
    recognitionRef.current.onstart = () => {
      console.log('Dual stream transcription started');
      setIsRecording(true);
    ...
    recognitionRef.current.onend = () => {
      console.log('Dual stream transcription ended');
      ...
      for (let i = event.resultIndex; i < event.results.length; i++) {
        ...
        if (event.results[i].isFinal) {
          ...
          const newSegment: TranscriptSegment = {
            ...
            confidence,
            isFinal: true,
            source: 'combined', // Combined audio from both sources
            speaker: 'Combined'
          };
    ...
    recognitionRef.current.onerror = (event: any) => {
      ...
      switch (event.error) {
        case 'no-speech':
          setError('No speech detected from either source. Please check audio levels.');
          break;
        case 'audio-capture':
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      console.error('Failed to start dual stream recording:', err);
  ...
  const updateUserVolume = (volume: number) => {
    setUserVolume(volume);
    if (userGainNodeRef.current) {
      userGainNodeRef.current.gain.value = userMuted ? 0 : volume;
    }
  };

  const updateMeetingVolume = (volume: number) => {
    setMeetingVolume(volume);
    if (meetingGainNodeRef.current) {
      meetingGainNodeRef.current.gain.value = meetingMuted ? 0 : volume;
    }
  };
  ...
  const toggleUserMute = () => {
    ...
    if (userGainNodeRef.current) {
      userGainNodeRef.current.gain.value = userMuted ? userVolume : 0;
    }
  };

  const toggleMeetingMute = () => {
    setMeetingMuted(!meetingMuted);
    if (meetingGainNodeRef.current) {
      meetingGainNodeRef.current.gain.value = meetingMuted ? meetingVolume : 0;
    }
  };
